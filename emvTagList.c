...
void emvparse2(unsigned char arr[], unsigned short size, tlvInfo_t * t, int * tindex, int index, dict_t *hashtab[HASHSIZE]){
		int j;
		t[*tindex].tlv =  *tlv_parse(arr, &index);
		printf("<ICCTag tagid=\"0x%X\">", t[*tindex].tlv.Tag);
		//printf("Len: %X\n", t[*tindex].tlv.Len);
		...
		*tindex +=1;
		...
		{ //several consecutive primite tlv's
			return emvparse2(&arr[index], size-index , t, tindex, 0 , hashtab);
		}
}
...
void emvFormat_result(tlvInfo_t* t, int tindex){
	int i,j;
	int tabs=1;
	int composedLen=0;
	int trackLen =0;
	...
	for(i=0; i<tindex; i++){
		if(trackLen >=composedLen) tabs--;
		printf("<ICCTag tagid=\"0x%X\">", t[i].tlv.Tag);
		if(t[i].PC == CONSTRUCTED){
			trackLen =0;
			composedLen = 0;
			composedLen = t[i].tlv.Len;
